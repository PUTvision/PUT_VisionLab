...
namespace PUTVison_CameraBasler
{
    ...
    public class CameraBasler : PUTVision_CameraBase.CameraBase, PUTVision_ICamera.ICamera
    {
        ...
        public void Capture(int _imageNumber, bool _enableImageSave, bool _enableImageShow)
        {
            // send acquire trgger to cameras
            for (int deviceIndex = 0; deviceIndex < NUM_DEVICES; ++deviceIndex)
            {
            ...
            // step 2 - save the image (here in simple, blocking way, later change to change to another thread with a buffer)
            if (_enableImageSave)
            {
            }

            // step 3 - show the image (should be done in new window)
            if (_enableImageShow)
            {
            }
        }
        // TODO - poprawić końcówkę z wyłapywaniem błędów, jest jakieś console.writeline ???
        public void Close()
        {
            // safely close the camera(s)
            try
            {
                /* Clean up. */
